...
export default function AccountDetailPage() {
  ...
  // Live balance: balanceThrough(id, currentMonthEnd()) provides the
  // checkpoint balance (which DOES NOT include the opening balance).
  // Add the opening balance from metadata on top, matching the fix
  // applied in AccountListPage (the user's fix: ob + cpBalance).
  const curMonthEnd = currentMonthEnd();
  const cpNow = id ? useCheckpointStore.getState().balanceThrough(id, curMonthEnd) : null;
  const totalBalance = cpNow !== null
    ? (metadataOpeningBalance ?? 0) + cpNow
    : (metadataOpeningBalance ?? 0) + transactions
