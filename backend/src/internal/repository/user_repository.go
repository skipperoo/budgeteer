...
func (r *UserRepository) Create(ctx context.Context, u *model.User) error {
	query := `INSERT INTO users (id, email, password_hash, public_key, encrypted_private_key, is_verified, created_at, updated_at)
...
func (r *UserRepository) FindByEmail(ctx context.Context, email string) (*model.User, error) {
	query := `SELECT id, email, password_hash, public_key, encrypted_private_key, is_verified, created_at, updated_at
	          FROM users WHERE email = $1`
	row := database.Pool.QueryRow(ctx, query, email)
	u := &model.User{}
	err := row.Scan(&u.ID, &u.Email, &u.PasswordHash, &u.PublicKey,
		&u.EncryptedPrivateKey, &u.IsVerified, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
